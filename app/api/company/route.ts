...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const response: CompanyApiResponse = {
      ...
      sources: {
        perfil: [
          ...(summary?.url ? [{ url: summary.url, excerpt: summary.summary }] : []),
          ...(wikidataId
            ? [{ url: `https://www.wikidata.org/wiki/${wikidataId}`, excerpt: "Ficha de Wikidata con industria y sitio web oficial." }]
            : [])
        ],
        salud: [{ url: "https://www.gdeltproject.org/", excerpt: "Señales públicas procesadas desde GDELT 2.1." }],
        estrategia: [...(summary?.url ? [{ url: summary.url, excerpt: "Contexto público para prioridades estratégicas." }] : [])],
        mercado: competitors.slice(0, 3).map((item) => ({ url: item.url, excerpt: `Competidor listado en Wikidata: ${item.name}.` })),
        laboral: [{ url: "https://www.gdeltproject.org/", excerpt: "Indicadores de sentimiento derivados de cobertura pública." }],
        noticias: [
          ...recentNews.slice(0, 5).map((item) => ({ url: item.url ?? "", excerpt: item.title })),
          ...controversyNews.slice(0, 5).map((item) => ({ url: item.url ?? "", excerpt: item.title }))
    ...
    // IMPORTANTÍSIMO: log para que veas el error real en Vercel
    console.error("[/api/company] failed:", err?.message ?? err);

    // Respuesta degradada (sin tumbar el frontend)
    const fallback: CompanyApiResponse = {
      company: { name, country, city },
      modules: {
        perfil: {
          industry: "N/D",
          ...
          employees: "N/D",
          revenueUSD: null,
          description: "No se pudo obtener información pública en este momento.",
          website: null
        },
        salud: { score: 0, kpis: [], chart: [], notes: ["Error consultando fuentes públicas."] },
        estrategia: { priorities: [], roadmap: [], strengths: [], weaknesses: [] },
        mercado: { marketShare: null, competitors: [], positioning: "N/D" },
        laboral: { turnover: "N/D", engagement: null, sentimentTrend: [], conditions: [], workEnvironmentSummary: "N/D", perceivedStability: "N/D" as any },
        noticias: { relevantEvents: [], newsGossip: [] }
      },
      rating: { score: 0, positives: [], risks: [], summary: "N/D" } as any,
      sources: { perfil: [], salud: [], estrategia: [], mercado: [], laboral: [], noticias: [] },
      updatedAt: now.toISOString()
    };

    // Devuelve 200 para que el UI cargue y muestre “N/D” en vez de error rojo.
